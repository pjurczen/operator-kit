...
description: >
  ...
  ask sounds settled enough to spec immediately. Runs BEFORE write-handoff; do not jump from
...
when_to_use: A NEW above-bar feature proposed ("we should build X", "make X real", "design X end to end"). Runs BEFORE write-handoff. NOT for below-bar work (bug fixes, chores, UI increments in an approved design) or a feature already designed in docs/features/.
---
...
`write-handoff` derive per-repo specs from it. The point: design decisions (auth model, data
...
skip straight to `write-handoff`. The bar cuts both ways: skipping design on an above-bar
...
- a **hardness verdict** — is this **hard** (a novel abstraction, no proven in-repo analog, or
  ...
  (`write-handoff`): the first deliverable is a thin slice putting the risky seam under a real
...
`write-handoff` derives one spec per repo **from the approved design** (specs cite it), then
`dispatch-handoff`. If design conversation surfaced work for a human (infra, accounts), queue
...
- **Encode your architectural invariants** — never a design that violates the constraints in
  [`CONFIG.md`](../../../CONFIG.md) §Architectural invariants.
