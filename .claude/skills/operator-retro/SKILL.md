...
when_to_use: Nightly scheduled run ("run operator-retro"), after a correction or a materially-wrong dispatched result (incident-scoped, run it then and there), judging due decision-log bets. AUDIT mode ("run the model audit/review") after model-changing decisions/refactors/correction-runs or monthly — never every tick. NOT for business-work review (that's the roadmap) or generating new work (find-opportunities).
---
