...
Live monitor state (armed = present below; anything MISSING → arm from [`monitors.md`](../../references/monitors.md)):
!`bash scripts/monitor-status.sh 2>/dev/null || echo "(monitors: fill in repos in scripts/monitor-status.sh)"`

Read dispatched-issue state, **reflect it on the status view** ([`board.md`](../../references/board.md)), and **surface every human action in the in-tick checklist** (`operator-cycle` step 4). Contract: [`OPERATOR_MODEL.md`](../../../docs/OPERATOR_MODEL.md).

Stance: **watch the issue, not the PR** — labels + closure + comments are the interface; PR/CI is the worker's lane, read **once per gate** (when `agent:in-review` appears) and when direct-polling merge state. **Be comment-aware** — load-bearing substance (corrections, deviations, questions, FYIs) lands as comments that fire no label monitor. Filter bot authors; skip your own echo (shared account). Arm monitors **only** from the canonical commands in [`monitors.md`](../../references/monitors.md).
