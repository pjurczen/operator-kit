...
# Active downstream repos come from the host's .claude/operator.json ("repos": ["org/a", "org/b"]);
# the canonical commands in .claude/references/monitors.md must encode the same set in their markers.
cfg="${CLAUDE_PROJECT_DIR:-$PWD}/.claude/operator.json"
[ -f "$cfg" ] || { echo "monitors: not configured (no .claude/operator.json — run /operator:init)"; exit 0; }
repos="$(python3 -c 'import json,sys; print(" ".join(r.split("/")[-1] for r in json.load(open(sys.argv[1])).get("repos", []) if isinstance(r, str)))' "$cfg" 2>/dev/null || true)"
[ -n "$repos" ] || { echo "monitors: not configured (repos list empty or invalid in .claude/operator.json)"; exit 0; }
