...
- **`/operator:design`** — the approval gate for an above-bar feature: a human-approved design before any spec. Runs BEFORE `spec`.
- **`/operator:spec`** — author the spec into `docs/handoffs/` (from the approved design, if above-bar).
- **`/operator:dispatch`** — open the labeled GitHub issue + reflect it on the status view.
- **`/operator:monitor`** — watch the dispatched issues, mirror state, triage `needs-triage`, proof-read merge gates, surface blockers.
- **`/operator:status`** — re-derive the living status doc from ground-truth.
- **Develop-passes:** **`/operator:retro`** (self-improvement + model audit) and **`/operator:opportunities`** (generate the backlog; proposes only).
...
Plugin scripts run through the host launcher: `bash scripts/operator <name>` (`monitor-status`, `bootstrap-labels <org>/<repo>`, `check`).
