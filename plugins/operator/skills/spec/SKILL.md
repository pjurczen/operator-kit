...
when_to_use: Turning a decided feature/fix into a spec ("write the handoff", "spec this for the worker", "prepare the downstream task"). NOT for deciding whether to build (decision log), designing an above-bar feature (design runs first), or opening the issue (dispatch).
---

# Write a handoff spec

Turn a **decided** priority into a self-contained spec at `docs/handoffs/<YYYY-MM-DD>-<slug>.md`, ready for `dispatch`. You specify *what and why* — never *how to write the code*; the downstream worker owns the how. The spec gets **inlined into a GitHub issue** at dispatch, so it must stand alone.
...
2. **Above the design bar?** (per `design`'s bar) → a **human-approved design must exist** in `docs/features/`. Derive the spec from it and cite it — the design owns the cross-repo contract and named tradeoffs; the spec carries them to one repo.

## Procedure
1. **Ground** on related decisions, prior specs, lessons (`docs/lessons/` or your knowledge store), and any number you'll cite via `.claude/references/data-sources.md` — verified data, never stale doc figures.
...
6. Output: the spec file. It's now ready for `dispatch`.

## Architectural discipline — the operator's value-add
- **Encode your architectural invariants** (`CONFIG.md` §Architectural invariants) as Hard constraints — a worker can't infer them, so name them explicitly in every spec they touch.
