...
when_to_use: Running operations, the recurring tick, "what should we do next", "run the loop", resuming operator work. Owns the general "what next" question — Decide invokes opportunities when candidates run short. NOT for a lone status check (monitor) or backlog generation alone (opportunities).
allowed-tools:
...
Live state at invocation (today's date; monitor liveness — arm anything MISSING from the canonical commands in `.claude/references/monitors.md`, never from memory):
!`date +%F`
...
## 1. Sense
- Run **`monitor`** — in-flight state, comments, triage, merge gates, across ALL active repos.
- Read your **living status doc** (**the current objective lives here**) + recent **decision log** (context + Revisit-due entries). Paths in `CONFIG.md`.
- Ground the tick's focus in what's already known (prior decisions, features, handoffs, `docs/lessons/` or your knowledge store — [`knowledge-store.md`](../../references/knowledge-store.md); if the store is central and down, halt and say so) — once per new focus; re-ground on topic shift.
...
- Make sure `retro` is scheduled to run nightly (a scheduled local run); re-create it if it's gone.
...
- Short on candidates or capacity idle → run `opportunities` (proposes only; the human approves into the roadmap).
- Pick the **single highest-value next action**, with its why.

## 3. Act
- Build work: above the design bar → **`design`** first (human-approved design) → `spec` → `dispatch` → **an immediate `monitor` pass**.
- **Fork on the hardness verdict:** bounded (known shape, proven analog) → dispatch + monitor as above; **hard** (novel abstraction / robustness-risk) → dispatch a **spike**, park the full build on the human's steer, and run `monitor`'s **tremor-watch** (not just label-watch) — the merge gate can't catch a wrong abstraction.
...
- Don't trust a scheduled tick to watch: run `monitor` after any dispatch **and** before ending a session with work in flight.
...
- Log **strategic** decisions only in the decision log. Material state change → update the living status doc (or `status` on cadence). Reflect roadmap statuses as items move.
...
- **Feature designs are never self-approved** — the `design` gate holds even when the human is away.
...
## Cadence
The `/loop` tick or a scheduled local run — subscription-billed, never API/Actions. Develop-speed companions: `retro` (nightly + on-demand the moment a process miss surfaces) and `opportunities` (weekly / on-demand).
