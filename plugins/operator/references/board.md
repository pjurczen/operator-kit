...
Shared mechanics for the **work-stream status view** — used by `dispatch` (reflect the dispatch) and `monitor` (update it as work moves). It is the **agent-work status view**, mirrored from GitHub issue/PR state. It is *not* the source of truth — the issues are. It just makes in-flight work legible.
...
Implement the status view however fits your operation — an **in-repo roadmap doc** (edited in place, versioned) or a **project board** (Asana / Linear / Jira via its MCP server). Put your choice + concrete IDs/path in `CONFIG.md`, not here.
...
## Configure
Record your board's identifiers (workspace/team/project or board/list IDs) and any status-field conventions in `CONFIG.md`. Never invent a target board silently — use the one your config names.
