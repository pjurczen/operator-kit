...
`scripts/check.sh` (check 6) asserts that every tool a skill declares in `allowed-tools` is covered here; add the same names to the skills that should call the store (`operator-cycle`, `designing-feature`, `write-handoff`, `operator-retro`).
